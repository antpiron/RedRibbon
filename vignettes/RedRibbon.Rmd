...
To demonstrate the capabilities of `RedRibbon`, we will apply it to multiple and diverse omic data sets. 
...
This data set is has been analyzed in depth in the RedRibbon main manuscript.
The data contains the output from 2 `DESeq2` differential expression analyses.
In this case, columns `a` and `b` contain log2FoldChange
values corresponding to type 2 diabetes signatures (`T2DvsCTL`) and human islets treated with palmitate and glucose signatures (`D8PGsCTL`). 
...
RedRibbon is capable of handling both long and short reads. To illustrate this, we provide an example in which we compare the signatures of long against short reads derived from beta pancreatic cells exposed to cytokines. 
The dataset for the long reads treated with cytokines can be accessed via the following link: GSE--. 
Similarly, the dataset containing the short reads is available at ---. 
Here, we use directly the outcomes of the differential analysis between cytokines-treated and control beta pancreatic cells, which are stored in `CYT-Short-Long-gene` and `CYT-Short-Long-tx`, for the gene and transcript expression respectively.

```{r mergeCYT}
data("CYT-Short-Long", package = "RedRibbon")

We will first compare the gene expression signatures between short and long reads.
...
gg <- ggRedRibbon(sh_vs_l_genes, quadrants=quad,
                  labels = c("short-reads CYT vs CTL", "long-reads CYT vs CTL"),
			      repel.force = 250) +
	coord_fixed(ratio = 1, clip = "off")
gg
```
...
df.tx <- merge(CYTvsCTL_short_tx[, c("enst", "log2FoldChange")],
                  CYTvsCTL_long_tx[, c("enst", "log2FoldChange")],
                 by = "enst")
...
gg <- ggRedRibbon(sh_vs_l_tx, quadrants=quad,
                  labels = c("short-reads CYT vs CTL (transcript level)", "long-reads CYT vs CTL (transcript level)"),
			      repel.force = 250) +
	coord_fixed(ratio = 1, clip = "off")
gg
```
...
One of the interesting overlap comparison is between transcriptome and proteome for the same cells and treatments.
Doing so, it shows that the change seen in transcriptome recapitulate in the proteome (or not). 
...
quad <- quadrants(rr, algorithm="ea", permutation=TRUE, whole=FALSE)
gg <- ggRedRibbon(rr, quadrants=quad,
      		  labels = c("Proteome Palm 24h", "Microarray 14h"),
		  repel.force = 250) +
   coord_fixed(ratio = 1, clip = "off")
gg
```

An nice overlap is observed on the major diagonal showing that most changes observed at transcript level recapitulate at protein level. 
Worth of note, in  [Lytrivi et al., 2020](https://bmcgenomics.biomedcentral.com/articles/10.1186/s12864-020-07003-0) with the original R RRHO package (Plaisier et al., 2010), we were unable to pinpoint the coordinate with the minimal hyper-geometric P-Value. Here, the improved accuracy of RedRibbon allows the detection of up-up quadrant overlap.
