...
RedRibbon will be applied to a synthetic data set (and later to a real data set see section `Real data set`) in this vignette for demonstration purpose.
...
non treated and treated conditions. Add r correlated genes,
...
We load a real dataset ([from this analysis](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE159984))
...
Then, RedRibbon is run

```{r RedRibbonT2DD8PG}
rr <- RedRibbon(df.real, enrichment_mode="hyper-two-tailed")
quad <- quadrants(rr, algorithm="ea", permutation=TRUE, whole=FALSE)
gg <- ggRedRibbon(rr, quadrants=quad, labels = c("T2D vs CTL", "D8PG vs CTL")) + coord_fixed(ratio = 1)
gg
